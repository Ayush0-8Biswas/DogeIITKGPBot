...
templates.menuTemplate = (prefix) => [
    ...
    },
    {
        ...
        "description": `Display A List Of Photo Filter Menu`,
...
templates.photoTemplate = (prefix, icon) =>
    `*「 PHOTO FILTER MENU 」*

${icon} ${prefix}pfilter _reply image_`;
...
templates.makerOptionsTemplate = function(optionList, prefix, text1, text2, sender2, botName, myTimezone, time) {
    ...
    for (let i of optionList) {
        ...
        thisOption.description = Object.keys(i.params).join(" ")
        if (text2 !== "undefined") {
    ...
    let desText;
    if (text2 === "undefined") {
        desText = `@${sender2.split('@')[0]}, Select from the options below for text ${text1}`
    } else {
        desText = `@${sender2.split('@')[0]}, Select from the options below for text ${text1 + " | " + text2}`
    }
    return {
        buttonText: 'Options: 📃',
        footerText: `*${botName}*`,
...
templates.photoFilterTemplate = function(optionList, prefix, imageBuffer, sender2, botName, myTimezone, time) {
    let optionRows = []
    for (let i of optionList) {
        let thisOption = {}
        thisOption.title = i.title
        thisOption.description = i.name
        thisOption.rowId = `${prefix}viofilter ${i.name} ${imageBuffer}`
        optionRows.push(thisOption)
    }
    console.log(optionRows)
    return {
